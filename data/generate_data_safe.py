...
if __name__ == "__main__":
    ...
    prompt_train = []
    chosen_train = []
    for j, i in tqdm(enumerate(beaver['330k_train'])):
        if i['is_safe']:
            prompt = prompt_template.format(raw_prompt = i['prompt'])
            prompt_train.append(prompt)
            chosen_train.append(prompt + i['response'])

    safe_train = {'prompt' : prompt_train,
                    'chosen' : chosen_train}
    safe_train_ds = Dataset.from_dict(safe_train)
    ...
    # safe_test = {'prompt' : prompt_test,
    #                 'chosen' : chosen_test}
    # safe_test_ds = Dataset.from_dict(safe_test)

    train_dl = DataLoader(safe_train_ds, batch_size = 512, shuffle = False)
    # test_dl = DataLoader(safe_test_ds, batch_size = 768, shuffle = False)
    ...
    train_ds_ = Dataset.from_dict(
        {
            'prompt' : prompt_train,
            'chosen' : chosen_train,
            'rejected' : full_train_responses
        }
    )
    train_ds_.save_to_disk('./pku_safe_train_llama3_init_pre.hf')
    ...
    train_ds.save_to_disk('./pku_safe_train_llama3_init.hf')
    ...
    # test_ds.save_to_disk('./pku_better_safe_llama3_init.hf')